...
namespace burgershack.Repositories
{
  public class MenuRepository
  {
    ...
    {
      // REVIEW 
      // dapper is creating an instance of the class so it requires a non-abstract class. the fix for today was to make the menu item class non abstract, you can still inherit from it but now it can be instantiated on its own and map the burgers/drinks/sides propertys over properly. 
      // In our use case here we need dapper to create them on the so it is okay to have it not abstract. ultimately if we wanted to keep the class as abstract, we would query our 3 tables independantly and then cast the results to MenuItem if we wanted that one big collection. this solution is more complicated and leads us down another rabbit hole.
